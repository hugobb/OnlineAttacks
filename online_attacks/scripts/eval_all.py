...
from online_attacks.classifiers import load_dataset, load_classifier, DatasetType, MnistModel, CifarModel
...
def eval_comp_ratio(logger, model_type, model_name, list_records=None):
    ...
    algorithm = create_algorithm(AlgorithmType.OFFLINE, params.online_params, N=len(dataset))
    ...
    for record_name in tqdm.tqdm(list_records):
        ...
        eval_results = {}
        eval_results["metrics"] = {name: defaultdict(list) for name in record["indices"]}
        ...
        for name, indices in record["indices"].items():
            ...
            comp_ratio = compute_competitive_ratio(permuted_indices, offline_indices)/len(offline_indices)
