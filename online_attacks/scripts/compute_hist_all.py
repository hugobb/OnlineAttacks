...
def compute_hist(logger, model_type, model_name, list_records=None):
    ...
    target_classifier = load_classifier(params.dataset, model_type, name=model_name, model_dir=params.model_dir, device=device, eval=True)
    ...
    for record_name in tqdm.tqdm(list_records):
        ...
        for name, indices in record["indices"].items():
            ...
            target_stream = datastream.BatchDataStream(dataset, batch_size=params.batch_size, transform=target_transform, permutation=permutation)
...
if __name__ == "__main__":
    ...
    launcher = Launcher(SlurmLauncher(compute_hist,  checkpointing=True), slurm=args.slurm)
    ...
    for exp_id in list_exp_id:
        ...
        for model_type in args.model_type:
            if args.random_eval:
                model_name_list = os.listdir(os.path.join(logger.path, "eval", model_type))
            else:
                model_type = model_type.value
                model_name_list = args.model_name
                if "all" in args.model_name:
                    list_files = os.listdir(os.path.join(OnlineAttackParams().model_dir, args.dataset.value, model_type))
