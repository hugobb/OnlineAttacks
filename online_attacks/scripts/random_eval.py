...
def random_model(model_dir, dataset, pattern="train_*.pth"):
    while True:
        ...
        list_models = glob.glob(os.path.join(model_dir, dataset.value, model_type.value, pattern))
        if len(list_models) > 0:
            break

    list_models = [os.path.splitext(os.path.basename(model))[0] for model in list_models]
...
class Run:
    def __init__(self, num_runs=1, same_model_type=False, pattern="train_*.pth"):
        self.same_model_type = same_model_type
        self.pattern = pattern
        self.num_runs = num_runs

    def create_random_config(self, params):
        params.model_type, params.model_name = random_model(params.model_dir, params.dataset, self.pattern)

        while True:
            model_type, model_name = random_model(params.model_dir, params.dataset, self.pattern)
...
if __name__ == "__main__":  
    ...
    run = Run(num_runs, args.same_model_type, args.pattern)
